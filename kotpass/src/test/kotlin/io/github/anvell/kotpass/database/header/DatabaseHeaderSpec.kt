...
import io.github.anvell.kotpass.resources.DatabaseHeaderRes
...
class DatabaseHeaderSpec : DescribeSpec({
    ...
    describe("Database header") {
        it("Properly reads KDF parameters") {
            val ver4Argon2 = decodeHeader(DatabaseHeaderRes.DbVer4Argon2)
            ...
            val ver4Aes = decodeHeader(DatabaseHeaderRes.DbVer4Aes)
            ...
            val ver3Aes = decodeHeader(DatabaseHeaderRes.DbVer3Aes)
            ver3Aes.fileHeaders.shouldBeInstanceOf<FileHeaders.Ver3x>()
        }

        it("Try to read/write header") {
            val ver4Argon2 = decodeHeader(DatabaseHeaderRes.DbVer4Argon2).let { header ->
                val buffer = Buffer()
